public class GuitarString {
    ...
    private RingBuffer buffer; // container for buffer(queue) elements

    // Creates a guitar string of the specified frequency,
    // using a sampling rate of 44,100.
    public GuitarString(double frequency) {
        final double SAMPLING_RATE = 44_100;
    ...
    public void tic() {
        final double ENERGY_FACTOR = 0.996;
        // average of the two samples
        double avgSample = (buffer.dequeue() + buffer.peek()) / 2;
        buffer.enqueue(ENERGY_FACTOR * avgSample);
    }
